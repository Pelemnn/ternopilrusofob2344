...
async def cmd_music(message: types.Message):
    ...
    if mp4_url:
        try:
            ...
            logger.warning(f"Не вдалося завантажити MP4 посилання ({e}), відправляємо з YouTube посиланням.")
    ...
    fallback_text = (
        ...
        f"👤 <i>{song['author']}</i>\n\n"
        f"📜 {song['chorus']}\n\n"
        f"🎧 <a href=\"{song['link']}\">▶️ Слухати / Дивитися на YouTube</a>\n\n"
        f"🇺🇦 <i>Слава Україні! Героям Слава!</i>"
    )
    await message.answer(fallback_text, parse_mode=ParseMode.HTML, disable_web_page_preview=False)
